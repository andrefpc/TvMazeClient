package com.andrefpc.tvmazeclient.presentation.xml_based.screen.main
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (pinRepository.getPin().isNullOrEmpty()) {
    ...
    private fun initListeners(canUseBiometrics: Boolean) {
        ...
        binding.enter.setOnClickListener {
            val pin = pinRepository.getPin()
            if (pin.isNullOrEmpty()) {
                pinRepository.savePin(binding.pin.text.toString())
                binding.enter.text = getString(R.string.login_button)
                openShows()
            } else {
    ...
    private fun openShows() {
        appNavigation.navigateTo(this, NavigatorScreen.Shows)
    }
